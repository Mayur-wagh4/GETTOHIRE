...
const PostJob = () => {
  ...
  const pollPaymentStatus = useCallback((transactionId) => {
    ...
    const checkStatus = async () => {
      ...
      let nextInterval = pollCount < 5 ? 3000 : pollCount < 15 ? 5000 : pollCount < 25 ? 10000 : 30000;
  ...
  }, [dispatch, restaurantDetails, validateForm, pollPaymentStatus]);

  const handleJobPosting = useCallback(async () => {
    if (!validateForm()) return;
  ...
  }, [dispatch, formData, isAbroad, navigate, validateForm, transactionId]);
  ...
  const handleTimeoutDialogClose = useCallback(() => {
  ...
  }, [paymentStatus, transactionId, pollCount]);
